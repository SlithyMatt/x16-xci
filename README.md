...
wait 394 # wait until last second
...
The only requires key for the title screen file is **duration**. The simplest possible title sequence would do nothing for zero jiffys, which would mean a file that only says ```duration 0```. This would just make the menu bar appear as soon as possible once the game program was run.  **duration** can only have a single number of jiffys for its value, with a maximum value of 65536 (1092.267 seconds, or 18 minutes and 12.267 seconds -- that would be a very long title screen, so try to keep it under a minute or 3600 jiffys).
