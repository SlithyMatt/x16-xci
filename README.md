...
* [Data Format](#data-format)
   ...
   * [Zone Files](#zone-files)
      * [Optional Keys](#zone-file-optional-keys)
...
* 40x30 map of 8x8 tiles. Each tile can have 16 colors, but also its own palette offset. The tiles provide a menu at the top of the screen, text display and controls at the bottom. Tiles may also act as static overlays on top of the background. Up to 704 unique tiles can be defined at a time.
* 128 individual 16x16 sprites, including a dynamic mouse cursor. Each sprite can have 16 colors, from its own palette offset. Up to 499 unique sprite frames can be defined at a time. All sprites except for the mouse cursor are confined to the bitmap area, and support collision with other sprites and tiles. The mouse cursor is context-sensitive, changing its frame based on its position and game state to indicate the type of action that will happen when the mouse button is clicked.
...
* **000L1B07.BIN** - This is the configuration data for level 1 of zone 0. It will be loaded to bank 7 when the game enters zone 0.
* **000L1B08.BIN** - This file contains the background bitmap for level 1 of zone 0. It will be loaded into banks 8-11 when the game enters zone 0, and into VRAM starting at 0:0500 when the game enters level 1.
* **000L1B12.BIN** - The music and sound effects for level 1 of zone 0. It will be loaded to bank 12 when the game enters zone 0. It is also the last file to be loaded into banked RAM for zone 0, as it only contains two levels. When starting a new game, or loading a game that left off in zone 0, the rest of the files below will not be loaded from the file system at the beginning of game play.
* **001L0B01.BIN, 001L0B02.BIN, 001L0B06.BIN** - All data for level 0 of zone 1. These will be loaded into banks 1-6 when the game enters zone 1.
* **001L1B07.BIN, 001L1B08.BIN, 001L1B12.BIN** - All data for level 1 of zone 1. These will be loaded into banks 7-12 when the game enters zone 1.
* **001L2B13.BIN, 001L2B14.BIN, 001L2B18.BIN** - All data for level 2 of zone 1. These will be loaded into banks 13-18 when the game enters zone 1. They are the last files to be loaded into banked RAM for zone 1.
* **002L0B01.BIN, 002L0B02.BIN, 002L0B06.BIN** - Zone 2, level 0
* **002L1B07.BIN, 002L1B08.BIN, 002L1B12.BIN** - Zone 2, level 1
* **002L2B13.BIN, 002L2B14.BIN, 002L2B18.BIN** - Zone 2, level 2
* **002L3B19.BIN, 002L3B20.BIN, 002L3B24.BIN** - Zone 2, level 3
* **002L4B25.BIN, 002L4B26.BIN, 002L4B30.BIN** - Zone 2, level 4
* **002L5B31.BIN, 002L5B32.BIN, 002L5B36.BIN** - Zone 2, level 5
* **002L6B37.BIN, 002L6B38.BIN, 002L6B42.BIN** - Zone 2, level 6
* **002L7B43.BIN, 002L7B44.BIN, 002L7B48.BIN** - Zone 2, level 7
* **002L8B49.BIN, 002L8B50.BIN, 002L8B54.BIN** - Zone 2, level 8
* **002L9B55.BIN, 002L9B56.BIN, 002L9B60.BIN** - Zone 2, level 9. These are loaded into banks 55-60, the highest RAM banks that can be populated by an XCI game, when the game enters zone 2.
...
* **tiles_hex** - Filename of the hex file containing the tile definitions. The format of this file will be explained in the [Tiles Hex File](#tiles-hex-file) section. Tile indices 0 through 255 will be available in all zones, but 256 and over are subject to change and may only be used reliably in the title screen or in zones that don't define custom tiles.
* **sprites_hex** - Filename of the hex file containing the sprite definitions. The format of this file will be explained in the [Sprites Hex File](#sprites-hex-file) section. Sprite frame indices 0 through 255 will be available in all zones, but 256 and over are subject to change and may only be used reliably in the title screen or in zones that don't define custom sprite frames.
...
* **init_cursor** - Sprite frame used as the initial mouse cursor.  The sprite frames start with index zero, which is the first 16x16 bitmap defined in **SPRITES.BIN**. This value must be between 0 and the highest sprite frame index, which could be as high as 498, depending on the size of **SPRITES.BIN**. However, it is strongly recommended to use an index lower than 256 to guarantee that the cursor  will be consistent between zones.
...
So how does XCI take the menu file and make that screen? Let's go through each of the required keys for it.  Just like with the main file, all of these keys need to be present, but after that we'll go through the optional keys that can be in there, which are also in the example. Any keys that aren't expected are ignored, just as if they were comments. If any of the values are invalid, the build will fail. It is important to note that tile and sprite indices higher than 255 are not guaranteed to be the same in every zone, so it is strongly recommended to use only indices 0 to 255 for any of these values.
...
* **menu_item** - This specifies an item to include in a menu. The items specified in this file will be placed in the last specified menu in order from top to bottom, with a maximum of 9 items per menu. The values available for items must come from one of the identifiers supported by XCI, but a game only needs to support a subset of these, however it must have at least one non-divider item per menu. The following identifiers are supported:
   ...
   * **save** - Rendered as "Save Game". This will overwrite the current saved game file. If the current session started from a new game and was never saved before, the player will be prompted for a new filename.
   * **saveas** - Rendered as "Save As...". This will prompt the player for a new filename to save the game via a dialog that has a text entry field, a "CLEAR" button to reset that field, and a "SAVE" button to write out the current progress. This gives the player the ability to maintain multiple restore points in case they are unsure about their most recent progress. No more than 8 files should be saved at a time, as the "Load Game..." dialog will only render that many. If the player tries to save to a new file when 8 already exist, the prompt for a new filename will remain. To save, the player must enter one of the current 8 filenames, which will then be overwritten.
...
The following keys are required, but based on their values some optional keys make become required. It is important to note that tile and sprite indices higher than 255 are not guaranteed to be the same in every zone, so it is strongly recommended to use only indices 0 to 255 for any of these values.
...
#### Zone File: Optional Keys

If the levels in a zone require different tiles or sprites for their animation than the default sets, then additional sets may be defined for the zone. The same keys are used as in the [**main file**](#main-file-required-Keys) to define these additional elements.

* **tiles_hex** - Defines the tiles for the zone starting at index 256. Indices 0 through 255 from the default tile set will still be available to the zone. If this key is not defined, the entire default tile set will be available to the zone. This will be the basis of binary file **000TILES.BIN** for zone 0, for example.
* **sprites_hex** - Defines the sprite frames for the zone starting at index 256. Indices 0 through 255 from the default frame set will still be available to the zone. If this key is not defined, the entire default sprite frame set will be available to the zone. This will be the basis of binary file **000SPRTS.BIN** for zone 0, for example.
...
### Sound Effects Files
Sound effects for XCI games must be 4578 Hz 8-bit PCM mono, loaded from a raw, uncompressed file. Most sound recording and editing tools can export in this format, or you can use a utility like [SoX](https://sourceforge.net/projects/soxr/) to convert a file in a different format. The sound effects files for each level are appended to the music file, which must fit in a single 8 kB RAM bank. This means that if you have a 2 kB music file, you only have 6kB remaining for sound effects, which would be a grand total of 1.3 seconds divided between them. Since most sound effects you would use are less than a second, you'd probably be able to add 2 to the level. You could omit music from a level and have all 8 kB dedicated to sound effects, which would give you 1.78 seconds. You're probably now starting to see why so few 8-bit games had sampled sound effects! They take up a lot of memory compared to synthesized sound. In addition to being short, this sample rate is very low fidelity, so intelligible speech is a bit difficult. XCI sound effects work best when they are bright and punchy.
